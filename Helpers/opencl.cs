...
namespace Template {
        ...
        public OpenCLImage(OpenCLProgram ocl, int width, int height) {
            ...
            if (itemType == typeof(int)) {
                ...
                GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba8, width, height, 0,
                    OpenTK.Graphics.OpenGL.PixelFormat.Rgb, PixelType.Int, texData);
                ...
                GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba32f, width, height, 0,
                    OpenTK.Graphics.OpenGL.PixelFormat.Rgb, PixelType.Float, texData);
            } else {
                ocl.FatalError("Unsupported OpenCLImage format");
            }

            texBuffer = ComputeImage2D.CreateFromGLTexture2D(ocl.context, ComputeMemoryFlags.WriteOnly,
                (int) TextureTarget.Texture2D, 0, OpenGLTextureID);
        }
    }
    ...
    public class OpenCLBuffer<T> where T : struct {
        ...
        public const int READ_ONLY_HOST = ON_HOST | ON_DEVICE | READ_ONLY;
        public const int RW_HOST = ON_HOST | ON_DEVICE | READ_WRITE;
    ...
    public class OpenCLProgram {
        ...
        public void SelectBestDevice() {
            ...
            if (bestPlatform > -1) {
                ...
                try {
                    ...
                    try {
                        context = new ComputeContext(ComputeDeviceTypes.Gpu, new ComputeContextPropertyList(platform),
                            null, IntPtr.Zero);
                    } catch {
        ...
        public OpenCLProgram(string sourceFile) {
            ...
            if (GLInteropAvailable) {
                clSource = clSource.Replace("// #define GLINTEROP", "#define GLINTEROP");
            }
    ...
    public class OpenCLKernel {
        ...
        public void SetArgument<T>(int i, T v) where T : struct {
            kernel.SetValueArgument(i, v);
        }

        public void SetArgument(int i, ComputeBuffer<int> v) {
            kernel.SetMemoryArgument(i, v);
        }

        public void SetArgument(int i, ComputeBuffer<float> v) {
            kernel.SetMemoryArgument(i, v);
        }

        public void SetArgument(int i, ComputeBuffer<uint> v) {
            kernel.SetMemoryArgument(i, v);
        }

        public void SetArgument(int i, OpenCLBuffer<int> v) {
            kernel.SetMemoryArgument(i, v._gpubuffer);
        }

        public void SetArgument(int i, OpenCLBuffer<byte> v) {
            kernel.SetMemoryArgument(i, v._gpubuffer);
        }

        public void SetArgument(int i, OpenCLBuffer<float> v) {
            kernel.SetMemoryArgument(i, v._gpubuffer);
        }
        ...
        public void SetArgument(int i, OpenCLImage<int> v) {
        ...
        public void Execute(long[] workSize, ComputeEventList events) {
            queue.Execute(kernel, null, workSize, null, events);
        }
